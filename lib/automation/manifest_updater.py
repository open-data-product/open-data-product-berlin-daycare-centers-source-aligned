...
def update_manifest(manifest_path, data_path):
    ...
    csv_output_port = next(
        (output_port for output_port in manifest["output_ports"] if
         output_port["id"] == "berlin-lor-daycare-centers-csv"),
        None)
    ...
    for subdir, dirs, files in sorted(os.walk(data_path)):
        ...
        for file in [file for file in files if not file.endswith("-cache.csv")]:
            file_url = f"https://raw.githubusercontent.com/open-lifeworlds/open-lifeworlds-data-product-berlin-lor-daycare-centers-source-aligned/main/data/{subdir}/{file}"
